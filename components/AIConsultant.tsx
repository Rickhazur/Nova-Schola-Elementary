...
import { Send, Bot, User, Sparkles, Image as ImageIcon, X, Globe, Search, Paperclip, ExternalLink, AlertTriangle, ShieldAlert, Lock, Mic, Volume2, PenTool, Layout, Minimize2, Video } from 'lucide-react';
...
const AIConsultant: React.FC = () => {
    ...
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognitionRef.current.onerror = (event: any) => {
        ...
        if (event.error === 'not-allowed') {
          alert("Por favor habilita el acceso al micrófono para hablar.");
        }
      };
  ...
  const speakText = async (text: string) => {
    ...
    // Filter text common to both methods
    ...
    // METHOD 1: Premium Voice (ElevenLabs)
    if (usePremiumVoice) {
      if (cleanText.length > 500 && !window.confirm("El texto es largo. ¿Usar créditos de voz Premium?")) {
        // Fallback to standard if user declines
      } else {
        try {
          setIsSpeaking(true);
          ...
          console.error("Premium Voice Failed, falling back to standard", e);

          let friendlyMsg = "Error desconocido de voz.";
          // Safe access to message property
          const msg = e?.message || "Unknown error";

          if (msg.includes("QUOTA_EXCEEDED")) friendlyMsg = "Créditos de ElevenLabs agotados. Usando voz estándar.";
          else if (msg.includes("INVALID_KEY")) friendlyMsg = "API Key de ElevenLabs inválida. Usando voz estándar.";
          else if (msg.includes("MISSING_KEY")) friendlyMsg = "Falta configurar API Key de ElevenLabs.";
          else friendlyMsg = `Error de Voz Premium: ${msg.substring(0, 50)}. Usando voz estándar.`;

          setCurrentWarningMsg(`⚠️ ${friendlyMsg}`);
          setTimeout(() => setCurrentWarningMsg(null), 6000);

          // Fallback continues below to standard voice
        }
      }
    }

    // METHOD 2: Standard Browser Voice (Fallback/Default)
    ...
    // Improved Voice Selection: Prioritize clear Spanish voices
    // Removed 'Samantha' as she is primary English
    const preferredVoices = [
      'Paulina', 'Monica', 'Jorge', 'Juan', 'Diego',
      'Google español', 'Microsoft Sabina', 'Microsoft Mexico'
    ];

    const selectedVoice = voices.find(v =>
      v.lang.startsWith('es') && preferredVoices.some(p => v.name.includes(p))
    ) || voices.find(v => v.lang.startsWith('es')); // Fallback to any Spanish voice

    if (selectedVoice) {
      utterance.voice = selectedVoice;
      if (selectedVoice.name.includes('Google')) utterance.rate = 0.95;
  ...
  const handleCaptureWhiteboard = () => {
    if (whiteboardRef.current) {
      const image = whiteboardRef.current.getSnapshot();
      if (image) {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden font-sans relative">
      ...
      <div className="p-4 border-b border-stone-100 bg-stone-50 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-stone-500 flex items-center gap-1">
              <Bot className="w-3 h-3" /> GPT-4o / 4o-mini
            </p >
          </div >
        </div >
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg font-bold text-xs transition-colors border ${showWhiteboard
              ? 'bg-indigo-100 text-indigo-700 border-indigo-200'
              : 'bg-white text-stone-600 border-stone-200 hover:bg-stone-50'
              }`}
            title={showWhiteboard ? "Ocultar Pizarra" : "Mostrar Pizarra"}
          >
            {showWhiteboard ? <Minimize2 className="w-3 h-3" /> : <PenTool className="w-3 h-3" />}
            {showWhiteboard ? 'Cerrar Pizarra' : 'Abrir Pizarra'}
          </button>

          {warnings > 0 && (
            <div className="flex gap-1 mr-2 px-2" title="Strikes / Advertencias">
              {[...Array(3)].map((_, i) => (
          ...
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-full font-bold flex items-center gap-1 transition-all border ${usePremiumVoice
          ...
          >
            {usePremiumVoice ? <Sparkles className="w-3 h-3 text-yellow-300" /> : <Volume2 className="w-3 h-3" />}
            {usePremiumVoice ? 'Voz Premium' : 'Voz Std'}
          </button>

          <button
            onClick={() => setUseSearch(!useSearch)}
            className={`text-xs px-3 py-1.5 rounded-full font-bold flex items-center gap-1 transition-all border ${useSearch
      ...
      < div className="flex-1 flex overflow-hidden" >
        ...
        {
          showWhiteboard && (
            <div className="flex-[2] border-r border-stone-200 bg-stone-100 p-2 flex flex-col min-w-0">
              <div className="flex-1 relative rounded-xl overflow-hidden shadow-sm border border-stone-200">
              ...
              <div className="pt-2 flex justify-end">
                ...
                >
                  <Send className="w-3 h-3" /> Enviar Pizarra al Tutor
                </button>
              </div>
            </div>
          )
        ...
        {/* CHAT PANEL */}
        <div className="flex-1 flex flex-col min-w-[320px] bg-stone-50/30">
            ...
            {messages.map((msg) => (
              ...
              >
                <div className={`flex max-w-[90%] gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${msg.role === 'user' ? 'bg-stone-800' : 'bg-teal-600'
                    }`}>
                  ...
                  <div className={`flex flex-col gap-2 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                    ...
                    {msg.image && (
                      <div className="rounded-xl overflow-hidden border border-stone-200 shadow-sm max-w-[200px] bg-white">
                        <img src={msg.image} alt="User upload" className="w-full h-auto" />
                      </div>
                    )}

                    <div className={`p-4 rounded-2xl shadow-sm text-sm leading-relaxed relative group ${msg.role === 'user'
                      ...
                      {msg.role === 'model' && !msg.isThinking && (
                        <button
                          ...
                          className="absolute -top-3 right-2 bg-white text-teal-600 border border-teal-100 p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-sm hover:bg-teal-50"
                          title="Escuchar"
                        >
          ...
          {/* SUGGESTED PROMPTS */}
          {!showWhiteboard && messages.length === 1 && (
          ...
          {/* INPUT AREA */}
          <div className={`p-4 bg-white border-t border-stone-100 ${showWhiteboard ? 'border-l border-stone-200' : ''}`}>
            ...
            <form onSubmit={handleSubmit} className="flex gap-2 items-end">
              ...
              <div className="flex flex-col gap-1">
                <button
                  ...
                  disabled={isLoading}
                  className={`p-3 rounded-xl transition-colors border ${attachedImage ? 'bg-teal-50 text-teal-600 border-teal-200' : 'bg-stone-50 text-stone-400 border-stone-200 hover:bg-stone-100'
                    }`}
                  title="Subir imagen"
                >
                ...
                <button
                  ...
                  className={`p-3 rounded-xl transition-all border ${isListening ? 'bg-rose-100 text-rose-600 border-rose-200 animate-pulse' : 'bg-stone-50 text-stone-400 border-stone-200 hover:bg-stone-100'
                    }`}
                  title="Usar Voz"
                >
              ...
              <div className="flex-1 relative">
                <textarea
                  ...
                  placeholder={attachedImage ? "Pregunta sobre la imagen..." : "Escribe o habla..."}
                  className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:bg-white transition-all text-stone-700 placeholder:text-stone-400 resize-none h-[96px]"
                  disabled={isLoading}
                />
                {useSearch && (
                  <div className="absolute right-3 bottom-3 text-blue-500" title="Búsqueda activa">
              ...
              <button
                ...
                className="bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white p-3 rounded-xl transition-colors flex items-center justify-center shadow-lg shadow-teal-200 h-[96px] w-[50px]"
              >
