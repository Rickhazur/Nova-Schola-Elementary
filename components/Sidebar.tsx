...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const t = {
    es: {
      ...
      progress: userRole === 'ADMIN' ? 'Panel Profe' : 'Mis Trofeos',
    ...
    en: {
      ...
      social: 'Game Arena',
      rewards: 'Prize Store',
      progress: userRole === 'ADMIN' ? 'Teacher Panel' : 'My Trophies',
  ...
  return (
    <aside className={`w-64 bg-white border-r-4 border-black h-screen fixed left-0 top-0 flex flex-col z-10 hidden md:flex font-sans transition-colors duration-300 shadow-comic-lg`}>
      <div
      ...
      <div className="p-4 border-t-4 border-black/10 space-y-3 bg-slate-50 shrink-0">
        ...
        <div className={`p-3 rounded-2xl border-2 border-black shadow-comic flex items-center gap-3 bg-white`}>
          <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 border-black bg-kid-blue text-white`}>
            <span className="text-sm font-black">{(userName || "U").charAt(0)}</span>
          </div>
          <div className="flex flex-col overflow-hidden">
            ...
            <span className={`text-[10px] uppercase tracking-wide font-bold text-kid-purple`}>
              {userRole === 'ADMIN' ? 'Profe' : 'Estudiante'}
            </span>
          </div>
        </div>
      </div>
