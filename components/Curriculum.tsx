...
const LiveClassroom: React.FC<LiveClassroomProps> = ({ session, studentName, onExit, language }) => {
  ...
  const initSession = async () => {
    ...
    // Init AI Session
    const sysPrompt = `Actúa como 'Nova Bot', un amigo robot divertido y paciente para niños de primaria. 
    Estudiante: ${studentName}. TEMA: ${session.topic}.
    Idiomas: ${language === 'en' ? 'Inglés' : 'Español'}.
    INSTRUCCIONES CLAVE:
    1. Sé muy entusiasta, usa emojis y explicaciones simples.
    2. Si el niño no entiende, da ejemplos con juegos o caricaturas.
    3. SIEMPRE usa la herramienta 'updateWhiteboard' para dibujar cosas visuales que expliquen el tema.
    4. Tu objetivo es que aprender sea divertido, no estresante.`;
